...
ROW_SCAN: 
    blt $zero, $s1, $imm1, $imm2, 256, COLUMN_SCAN  # Loop over screen rows
    beq $zero, $zero, $zero, $imm2, 0, TERMINATE    # stop program after last row
COLUMN_SCAN: 
    blt $zero, $s2, $imm1, $imm2, 256, PIXEL_CALC   # Loop over screen columns
...
DRAW_PIXEL: 
    ble $zero, $a0, $s0, $imm2, 0, IN_CIRCLE        # Pixel inside circle if within radius squared
OUT_CIRCLE: 
    add $a2, $imm1, $zero, $zero, 0, 0              # set pixel black 
    beq $zero, $zero, $zero, $imm2, 0, DRAW         # Proceed to draw the pixel
IN_CIRCLE: 
    add $a2, $imm1, $zero, $zero, 255, 0            # set pixel white 
DRAW: 
    mac $t0, $s1, $imm1, $s2, 256, 0                # calc pixel index
